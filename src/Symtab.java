...
public class Symtab {
    ...
    public static void display_tab(ArrayList<VariableType> tab, int num) { //打印参数表和变量表
        for (int i = 0; i < num; i++) {
            if (tab.get(i).isArray) {
                iComplier.pr_sym.print("   name: " + tab.get(i).name + " type: " + tab.get(i).type + " value: " + tab.get(i).value
                ...
                if (tab.get(i).point_level > 1)
                    iComplier.pr_sym.print(" isPointer: " + tab.get(i).isPointer + " pointerLevel: " + (tab.get(i).point_level - 1));
                iComplier.pr_sym.print("\n");
            } else {
                iComplier.pr_sym.print("   name: " + tab.get(i).name + " type: " + tab.get(i).type + " value " + tab.get(i).value
                        + " isStatic: " + tab.get(i).isStatic + " isConst: " + tab.get(i).isConst);
                if (tab.get(i).isPointer)
                    iComplier.pr_sym.print(" isPointer: " + tab.get(i).isPointer + " pointerLevel: " + tab.get(i).point_level);
                iComplier.pr_sym.print("\n");
            }
        }
    }
    ...
    public static void display_global_var_tab() throws Exception { //打印全局变量表
        iComplier.pr_sym.print("GLOBAL_VARIABLE {\n");
